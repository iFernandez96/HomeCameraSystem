...
| PR-202 | Encrypt backups before publication. Use a recipient public key on the Jetson, keep the recovery key off-device, schedule local encrypted backups, and record backup age plus the explicit replication state. Genuine automatic off-device replication is excluded from this PR and tracked by PR-208; a same-Jetson drive or intermittently present laptop is not an acceptable substitute. | P0 | Done | Engineering (Codex) + Operator for key custody | PR-201 | Backup bytes disclose no filenames, keys, users, or configuration; tampering fails closed; no plaintext final or temporary survives; a clean scratch restore meets RTO <=60 min; status explicitly identifies off-device replication as deferred. | 2–3 d | [`server/app/services/backup_archive.py`](../server/app/services/backup_archive.py), [`server/app/services/backup_crypto.py`](../server/app/services/backup_crypto.py), [`server/app/services/backup_status.py`](../server/app/services/backup_status.py), [`server/app/routes/control.py`](../server/app/routes/control.py), [`deploy/docker-compose.yml`](../deploy/docker-compose.yml), [`deploy/RECOVERY_DRILLS.md`](../deploy/RECOVERY_DRILLS.md), [`docs/decisions/pr-202-encrypted-local-backups.md`](decisions/pr-202-encrypted-local-backups.md) |
| PR-203 | Make restore maintenance mode honest and application-wide, or narrow the documented guarantee. Reject normal mutations while replacement/validation runs and expose typed maintenance state. | P1 | Done | Engineering (Codex) | PR-201 | Concurrent maintenance operations and ordinary mutations are rejected with typed responses; reads can report maintenance; failed validation restores pre-restore bytes; restart cannot leave a stale in-process maintenance flag. | 0.5–1 d | [`server/app/services/backup_restore.py`](../server/app/services/backup_restore.py), [`server/app/main.py`](../server/app/main.py), [`server/app/routes/control.py`](../server/app/routes/control.py), [`server/tests/test_maintenance_mode.py`](../server/tests/test_maintenance_mode.py), [`server/tests/harness_backup/`](../server/tests/harness_backup/) |
| PR-204 | Implement the low-duty synthetic WHEP probe already designed. Require RTP/frame evidence, expose TTFF/result metrics, and route local failures into the existing persisted recovery ladder rather than creating another ladder. | P0 | Done | Engineering (Codex) | PR-101, PR-102 | Every rung is probed at bounded cadence; signaling-success/no-media is detected; debounce produces exactly one existing ladder action; external-only cellular failure alerts without restarting the camera; the worker/server/client metric contract remains synchronized. | 2–3 d | [`detection/whep_probe.py`](../detection/whep_probe.py), [`detection/mediamtx_watchdog.py`](../detection/mediamtx_watchdog.py), [`detection/metrics.py`](../detection/metrics.py), [`server/app/routes/_internal.py`](../server/app/routes/_internal.py), [`server/app/routes/metrics_prom.py`](../server/app/routes/metrics_prom.py), [`server/app/services/whep_probe_status.py`](../server/app/services/whep_probe_status.py), [`client/src/lib/telemetry.ts`](../client/src/lib/telemetry.ts), [`deploy/prometheus/alerts.yml`](../deploy/prometheus/alerts.yml) |
| PR-205 | Add bounded ongoing host supervision for the server container, distinct from camera recovery. Debounce health failures, restart only the server tier, and stop/alert on structural loops. | P0 | Done | Engineering (Codex) | PR-001 | Killing the server container recovers service within two minutes; camera publication continues; repeated structural failure stops after a bounded count and alerts; the action and reason are visible separately from camera recovery. | 1–1.5 d | [`deploy/server_supervisor.py`](../deploy/server_supervisor.py), [`deploy/systemd/homecam-server-supervisor.service`](../deploy/systemd/homecam-server-supervisor.service), [`deploy/systemd/homecam-server.service`](../deploy/systemd/homecam-server.service), [`deploy/docker-compose.yml`](../deploy/docker-compose.yml), [`deploy/recovery-drill.sh`](../deploy/recovery-drill.sh), [`server/tests/test_server_supervisor.py`](../server/tests/test_server_supervisor.py), [`server/app/routes/healthz.py`](../server/app/routes/healthz.py) |
| PR-206 | Deliver critical alerts off-box. Add a receiver for degraded-but-online failures and keep the Android health monitor as the independently running Jetson-offline detector. Add backup age, mount, WHEP probe, root disk, update/restore, and server-restart alerts. | P1 | Done | Engineering (Codex) + Operator | PR-202, PR-204, PR-205 | Each critical alert is injected in a drill and arrives once off-box; recovery notice arrives; annotations are secret-safe; Jetson power-off produces an Android alert without the server running. | 1–2 d | [`server/app/alert_receiver.py`](../server/app/alert_receiver.py), [`server/app/services/operational_metrics.py`](../server/app/services/operational_metrics.py), [`deploy/prometheus/alerts.yml`](../deploy/prometheus/alerts.yml), [`deploy/alertmanager/alertmanager.yml`](../deploy/alertmanager/alertmanager.yml), [`deploy/alert-drill.sh`](../deploy/alert-drill.sh), [`deploy/docker-compose.grafana.yml`](../deploy/docker-compose.grafana.yml), [`android-wrapper/src/main/java/com/example/homecamerasystem/JetsonHealthMonitor.java`](../android-wrapper/src/main/java/com/example/homecamerasystem/JetsonHealthMonitor.java) |
...
PR-202 completion note (2026-07-13): implementation commit `a924796` added a
streaming X25519/HKDF-SHA256/AES-256-GCM envelope, atomic ciphertext-only
publication, fail-closed authenticated restore, mode-0600 status and artifacts,
14-artifact retention, stale-plaintext cleanup, cross-process maintenance
serialization, daily persistent systemd scheduling, plaintext migration, and a
bounded scratch-recovery CLI. The recovery private key was generated on the
recovery laptop with mode 0600; only its public recipient key was provisioned
to the Jetson, and live host/container checks found no private-key file or
environment path. Contracts, client lint/typecheck/1,620 tests/build, server
1,399 tests (85 expected hardware/fixture skips), detection 672 tests (23
expected skips), Android debug assembly, Compose, shell, and focused recovery
checks passed. ARM64 image `sha256:2f5d379a851d...` was activated with zero
restarts. A fresh live ciphertext restored ten files to clean scratch in 0.297
seconds and disclosed none of the tested filename/config/user markers; a
one-byte mutation failed authentication and left zero plaintext files. Both
legacy plaintext backups migrated successfully and independently restored to
scratch in 0.248 and 0.309 seconds. The Jetson now retains four `.hcbk` files,
zero plaintext/manifest/temp backup files, and an enabled healthy daily timer;
status reports `deferred_off_device`. Genuine automatic off-device replication
and its 24-hour RPO were deliberately excluded and remain open as PR-208.

PR-203 completion note (2026-07-13): implementation commit `9d12152` added a
race-safe in-process mutation gate around the restore replacement/validation
window. Ordinary POST/PUT/PATCH/DELETE requests now receive a typed 503 with
`Retry-After`, concurrent maintenance operations receive a typed 409, and safe
reads remain available. `/api/status` exposes the typed active operation and
whether it blocks mutations; backup continues to use its online-snapshot write
semantics while remaining serialized against restore. Backup and restore work
runs in FastAPI's threadpool so the event loop can reject concurrent requests
during the real window, and lifespan startup explicitly clears process-local
state. Focused maintenance, rollback, status-contract, and control checks passed
(110 tests); client lint/typecheck, 1,620 tests, and production build passed;
the complete server suite passed (1,410 tests, 85 expected hardware/fixture
skips); contract and clean-source checks passed. No phone install or Jetson
activation was performed because PR-203 changes only local server/API behavior.

PR-204 completion note (2026-07-13): implementation commits `330f51f` through
`4be8e7b` added a serialized recv-only WHEP probe with an 8-second total bound,
60-second primary cadence, five-minute adaptive cadence, first-RTP TTFF/result
metrics, typed signaling-success/no-media detection, and Python 3.6-compatible
JetPack 4/GStreamer 1.14 ICE and BUNDLE negotiation. Three recoverable failures
queue exactly one action through the existing persisted watchdog ladder and its
cooldown; probe-backend and grant failures cannot restart the camera. An
authenticated, bounded cellular-client signal drives a separate external-only
alert and imports no camera recovery path. Worker grant tokens are exact-path,
one-use, omitted from logs, and all SDP diagnostics redact credentials,
addresses, and candidate values. Generated worker/server/client contracts,
Prometheus metrics, and seven alert rules are synchronized and validated.
Contracts and release-source checks, client lint/typecheck/1,622 tests/build,
server 1,415 tests (85 expected fixture/hardware skips), detection 688 tests
(23 expected skips, including the Python 3.6 guard), Android debug assembly,
and Prometheus config/rules all passed. On the deployed Jetson, the production
scheduler repeatedly received RTP on `cam` (0.5-1.8 s), `cam_lq` (4.0-7.1 s),
and `cam_uq` (4.2-4.4 s), all below 8 seconds; metrics reported success, zero
consecutive failures, and zero stream-stale restarts. Server, detection, and
MediaMTX services remained active and `/healthz` returned healthy. No Android
device was connected, so no APK was installed; the hardware-required WebRTC
gate was satisfied directly on the live Jetson.

PR-205 completion note (2026-07-13): implementation commits `2c35f0f` through
`45bc93a` added a Python 3.6-compatible host supervisor that checks the
server's loopback `/healthz` every ten seconds and requires three consecutive
failures before recreating only the Compose `server` service. The supervisor
persists its action budget before the side effect in a private mode-0600 JSON
state file. Three restart actions in ten minutes exhaust the budget; the next
debounced failure exits 78, remains failed through systemd's
`RestartPreventExitStatus`, and emits `alert=structural_loop action=stop`.
Docker's independent restart policy is disabled so it cannot bypass that
circuit breaker. State and journald expose server-specific status, action,
reason, result, and restart history without importing or invoking MediaMTX,
Argus, detection-worker, or reboot recovery. Contracts/release-source checks,
Compose and shell validation, 12 focused supervisor tests, and the full server
suite (1,427 passed, 85 expected hardware/fixture skips) passed. The Jetson's
host Python 3.6 compiled and ran the supervisor; committed/deployed supervisor,
Compose, drill, and unit checksums matched. In the live kill drill, three failed
probes produced one server-only recreation and the replacement reached Docker
healthy inside the enforced 120-second bound while MediaMTX and detection PIDs
remained unchanged and RTSP still returned H.264. With three actions already
inside the window, a further outage latched in 32 seconds, emitted the local
structural-loop alert, performed no fourth restart, left both camera-service
PIDs unchanged, and allowed the live camera snapshot to advance by 29 seconds.
...
by alert name and drill identity, sends recovery webhooks, and retries when no
subscription accepts a delivery. The receiver mounts the subscription/key
registry read-only, validates a one-alert bounded payload, never logs request
bodies or endpoint tokens, and returns a retryable failure until at least one
off-box handoff succeeds. Client lint/typecheck/build and 1,622 tests, 1,444
...
healthy. The connected S24 wrapper smoke passed. The final hardware drill on
2026-07-14 began with notification permission granted, a successful independent
phone probe, zero consecutive failures, and `offline_notified=false`. A clean
Jetson power-off then removed SSH and every Jetson process. Two forced executions
of the persisted Android JobScheduler job, with the Jetson still off, recorded
failure one and failure two, changed `offline_notified` to true, and produced the
operator-confirmed **Jetson offline or unreachable** notification. After the
operator physically restored power, `/healthz` returned, a forced phone check
reset failures to zero and `offline_notified` to false, and the operator
confirmed **Jetson is back online**. MediaMTX, detection, server supervision,
Prometheus, Alertmanager, and the receiver returned healthy; a final RTSP probe
decoded H.264 at 1280x720. Every PR-206 acceptance criterion is therefore
verified. Physical mount identity and fallback-write prevention remain PR-207;
PR-206 alerts on an unavailable recording-storage probe and server outage.
...
| ID | Minimum necessary change | Pri | Status | Owner | Dependencies | Acceptance criteria | Estimate | Relevant code/docs |
|---|---|---:|---|---|---|---|---:|---|
| PR-301 | Pin server base images, Python dependencies with hashes, downloaded Compose/MediaMTX checksums, and GitHub Actions commit SHAs. Produce an SBOM and retain security results with the release. | P1 | Awaiting evidence | Release engineering (Codex) | PR-000 | Clean builders resolve the same dependency set; unverified downloads fail; SBOM and vulnerability results identify the artifact; normal and security CI are green. | 1.5–2.5 d | [`deploy/Dockerfile.server`](../deploy/Dockerfile.server), [`server/requirements.in`](../server/requirements.in), [`server/requirements.txt`](../server/requirements.txt), [`deploy/download-verified.sh`](../deploy/download-verified.sh), [`deploy/install-jetson.sh`](../deploy/install-jetson.sh), [`scripts/generate-security-artifacts.sh`](../scripts/generate-security-artifacts.sh), [`.github/workflows/checks.yml`](../.github/workflows/checks.yml), [`.github/workflows/security.yml`](../.github/workflows/security.yml) |
...
PR-301 implementation note (2026-07-14): commit `aed3f9f` pins the production
Python base by digest, installs hash-locked runtime and test dependency sets,
verifies exact Compose and MediaMTX downloads before replacement, and pins every
GitHub Action to a full commit SHA. The security workflow cross-builds the real
ARM64 server image and retains its CycloneDX SBOM, vulnerability/secret reports,
build metadata, and artifact identity for 30 days. Local CI-equivalent suites,
tamper rejection, clean Python 3.11/3.12/pip resolution, ARM64 runtime imports,
and the security gate passed. Clean evidence identified source `aed3f9f`,
fingerprint `500de136eccfc0fe`, image digest `sha256:9f48c5093522dff3de09540651e026fd0e93077ff1aeaaa70c5d41e328f41221`,
and `dirty:false`; both scans contained zero high/critical fixed findings and
zero secrets. Status remains `Awaiting evidence` only until the pinned hosted
normal and security workflows pass for this branch; no Jetson or phone change
is required for PR-301.
...
| 2026-07-14 | PR-206 | `release/pr-206-offbox-alerts`; `14af87e`, `807bd1d`, `815109c` | Local suites and alerting validators passed. Isolated drill `pr206-1784013646` recorded exactly one firing and recovery handoff for each of 15 paths with no synthetic residue. During a real power-off, two phone-owned checks independently entered offline state and the operator confirmed the Android offline notification. After physical power restoration, the phone reset to healthy and the operator confirmed its recovery notification; all services recovered and RTSP decoded H.264 at 1280x720. | Done |
| 2026-07-14 | PR-301 | `release/pr-301-pinned-supply-chain`; `aed3f9f`; `/tmp/homecam-pr301-aed3f9f` | Hash locks reproduced byte-for-byte; Python 3.11, Python 3.12, and pip builders resolved the same 51 versions; tampered downloads failed without replacing verified bytes; client (1,622), server (1,445), detection (688), Android, contracts, and targeted cryptography/backup tests passed. The clean ARM64 artifact imported production modules, omitted build tools, and recorded `dirty:false`, source/fingerprint, archive checksum, and container digest. CycloneDX SBOM and image/source reports were retained locally with zero high/critical fixed vulnerabilities and zero secrets. Hosted pinned workflows have not run because the branch has not been pushed. | Awaiting evidence |
