...
- Daily timelapse (`services/timelapse.py` + `routes/control.py`, the "Build video" / "stitch all captures" feature). Hard-won invariants — don't regress:
  ...
  - Build writes to a `<day>.mp4.tmp` sidecar (needs `-f mp4`, can't muxer-infer `.tmp`) then ATOMIC `os.replace` → final, so the GET route never serves a partial; cleanup `.tmp` on every failure path. ffmpeg timeout SCALES with clip count (`_ffmpeg_timeout_for`, 2.5 s/clip, 120 s–30 min) — a busy day is 300+ clips / multi-GB / 10+ min on the Nano.
