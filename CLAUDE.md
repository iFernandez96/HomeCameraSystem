...
- `/api/_internal/*` worker routes require the shared file-backed worker bearer credential plus an exact direct peer (`127.0.0.1`, `::1`, or the fixed HomeCam Docker gateway). Reject proxy-marker headers before credential checks, return empty 401/403/503 responses, and never log credential bytes. The MediaMTX callback is on a separate exact-peer router and remains credential-free; it also rejects proxy markers.
- `/api/_internal/detection/config` mirrors the user-facing GET for the authenticated worker config poll. Keep BOTH routes. The host worker and optional audio watcher must refuse startup before acquiring hardware if `/etc/homecam/worker-auth.secret` is missing or malformed.
- `/metrics` and `/healthz` live at root, not `/api/*`. Scrapers/probes don't speak cookies. PR-105 source-gates `/metrics` to loopback plus the fixed `172.30.0.0/24` HomeCam Compose network and returns an unknown-route 404 to remote callers; `/healthz` remains remotely reachable for liveness. Never broaden metrics to a tailnet/LAN range or publish Prometheus/Grafana ports remotely.
...
- Canonical client address (PR-104): application code uses only normalized `request.client.host`; it never parses forwarding headers. Production Uvicorn accepts `X-Forwarded-For`/`Proto` only from literal `FORWARDED_ALLOW_IPS=127.0.0.1,::1,172.30.0.1` (loopback plus the fixed HomeCam Docker gateway). Never use `*`, the whole Docker subnet, or a tailnet range. Tailscale Serve is the header-setting HTTPS proxy; MediaMTX remains host-native with its separate loopback-only trusted-proxy list. Login backoff is endpoint + NFKC/strip/casefold account + canonical source, persisted in bounded `audit.db` state; no global middleware or whole-house lockout. Decision: `docs/decisions/pr-104-trusted-client-address.md`.
...
- MediaMTX v1.18 uses `authMethod: http` with an empty exclusion list. The callback accepts only exact source IPs in `MEDIAMTX_AUTH_TRUSTED_CALLERS` (loopback plus the fixed `172.30.0.1` HomeCam Docker gateway), never logs/reflects callback credentials, and allows loopback RTSP for exact internal video/audio paths. Every remote WebRTC video read and remote `publish talk` / `read listen` requires an exact-path opaque 60-second one-use token sent only in `Authorization`; authenticated users may request registered video-read grants, talk issuance remains owner-only, and audio issuance/consumption rechecks `audio_enabled` plus Privacy. Raw media tokens are transient client locals, retained server-side only as SHA-256 digests, and must never enter state, logs, telemetry, URLs, query strings, or evidence. MediaMTX browser origins are a finite allowlist, never `*`.
...
- `POST /api/client-log` (`routes/client_log.py`): bounded unauth sink for the PWA's `lib/log.ts` ship() — device-side error/warn lands in journald with a `client_log:` prefix. `extra='forbid'` + global app-level rate cap (`_CLIENT_LOG_MAX_PER_WINDOW`, NOT middleware). Keep it outside the worker router so it works on the anon login screen without inheriting worker trust.
